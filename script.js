class Paint {
  ...
  init() {
    ...
    // this.svg.addEventListener('click', this._onClickSvg.bind(this))
    this.addEventsSvg();
  }

  addEventsSvg() {
    // this.svg.addEventListener('click', this._onClickSvg.bind(this))
    this.svg.onclick = this._onClickSvg.bind(this)
  }

  removeEventsSvg() {
    // this.svg.removeEventListener('click', this._onClickSvg.bind(this))
  ...
  _onClickSvg(e) {
    ...
    this.currentElement.create(position.x, position.y)
  }
}
...
class Rect extends Figure {
  init() {
    this._element = document.createElementNS(this._svgns, 'rect');
  }
  create(x, y) {
    this._element.setAttributeNS(null, 'data-draggable', '');
    this._element.setAttributeNS(null, 'x', x);
    this._element.setAttributeNS(null, 'y', y);
    this._element.setAttributeNS(null, 'height', '50');
    this._element.setAttributeNS(null, 'width', '50');
    this._element.setAttributeNS(null, 'fill', '#'+Math.round(0xffffff * Math.random()).toString(16));
    this.svg.appendChild(this._element)
  }
}

class Ellipse extends Figure{
  init() {
    this._element = document.createElementNS(this._svgns, 'circle');
  }
  create(x, y) {
    this._element.setAttributeNS(null, 'data-draggable', '');
    this._element.setAttributeNS(null, 'cx', x);
    this._element.setAttributeNS(null, 'cy', y);
    this._element.setAttributeNS(null, 'r', '50');
    this._element.setAttributeNS(null, 'fill', '#'+Math.round(0xffffff * Math.random()).toString(16));
    this.svg.appendChild(this._element)
  }
}
